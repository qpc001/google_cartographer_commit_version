...
public:
    ...
    XYIndexRangeIterator(const Eigen::Array2i& min_xy_index,
        ...
        : min_xy_index_(min_xy_index),  //最小的 x,y
          max_xy_index_(max_xy_index),  //最大的 x,y
          xy_index_(min_xy_index) {}    //迭代器从 最小值x,y开始

    // Constructs a new iterator for everything contained in 'cell_limits'.
    // 构造迭代器: 对给定的cell_limits, 遍历里面的元素
    ...
    XYIndexRangeIterator& operator++() {
        ...
        DCHECK(*this != end());
        // 按行遍历
        if (xy_index_.x() < max_xy_index_.x()) {    // 如果横坐标还没有到最大值,则继续增加
            ++xy_index_.x();
        } else {
            xy_index_.x() = min_xy_index_.x();      // 否则, 纵坐标+1
            ++xy_index_.y();
        }
        return *this;
    }
    ...
    //判断{x,y}是否等于输入的{other.xy_index_}
    bool operator==(const XYIndexRangeIterator& other) const {
        return (xy_index_ == other.xy_index_).all();
    }

    //判断{x,y}是否不等于输入的{other.xy_index_}
    bool operator!=(const XYIndexRangeIterator& other) const {
        return !operator==(other);
    }

    // 返回一个新的迭代器, 重新构造一个新的迭代器(最小xy , 最大xy)
    ...
    XYIndexRangeIterator end() {
        XYIndexRangeIterator it = begin();  //先得到一个新的迭代器
        //然后把迭代器当前索引指向 (x_min, y_max+1) 也就是最后一行再加1行的起始
        it.xy_index_ = Eigen::Array2i(min_xy_index_.x(), max_xy_index_.y() + 1);
        return it;
    }

private:
    Eigen::Array2i min_xy_index_;   //最小的xy
    Eigen::Array2i max_xy_index_;   //最大的xy
    Eigen::Array2i xy_index_;       //当前的xy
};
