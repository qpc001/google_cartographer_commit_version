...
proto::LocalTrajectoryBuilderOptions2D CreateLocalTrajectoryBuilderOptions2D(
  ...
  options.set_min_range(parameter_dictionary->GetDouble("min_range"));      //激光最近距离
  options.set_max_range(parameter_dictionary->GetDouble("max_range"));      //最远
  options.set_min_z(parameter_dictionary->GetDouble("min_z"));              //z方向最小值
  options.set_max_z(parameter_dictionary->GetDouble("max_z"));              //z方向最大值
  options.set_missing_data_ray_length(
      parameter_dictionary->GetDouble("missing_data_ray_length"));          //如果某条射线没有返回,那么设置该miss点的距离
  options.set_num_accumulated_range_data(
      parameter_dictionary->GetInt("num_accumulated_range_data"));          //累积的激光数量
  options.set_voxel_filter_size(
      parameter_dictionary->GetDouble("voxel_filter_size"));                //体素滤波器size
  options.set_use_online_correlative_scan_matching(
      parameter_dictionary->GetBool("use_online_correlative_scan_matching"));   //使用在线的相关性扫描匹配
  *options.mutable_adaptive_voxel_filter_options() =
      sensor::CreateAdaptiveVoxelFilterOptions(
          parameter_dictionary->GetDictionary("adaptive_voxel_filter").get());  //自适应滤波
