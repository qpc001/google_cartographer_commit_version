...
public:
    ...
    // Aligns 'point_cloud' within the 'probability_grid' given an    通过初始位姿进行旋转\平移,使得激光扫描与概率地图对齐,返回位姿估计
    // 'initial_pose_estimate' then updates 'pose_estimate' with the result and
    // returns the score.
    // 输入{初始位姿(机器人在子图的坐标) , 激光扫描 , 子图概率地图}
    // 对激光扫描进行旋转\平移匹配, 得到得分最高的候选解, 输出: 位姿估计pose_estimate
...
private:
    ...
    const proto::RealTimeCorrelativeScanMatcherOptions options_;      //ScanMatcher配置选项
};
