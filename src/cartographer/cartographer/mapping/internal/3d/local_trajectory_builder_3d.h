...
// 3D local SLAM的核心部分(暂不看)
class LocalTrajectoryBuilder3D {
public:
    // 插入结果
    struct InsertionResult {
        std::shared_ptr<const mapping::TrajectoryNode::Data> constant_data;         //节点数据
        std::vector<std::shared_ptr<const mapping::Submap3D>> insertion_submaps;    //插入的子图列表
    };
    // 匹配结果
