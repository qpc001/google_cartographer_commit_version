...
///////////////////////////////////////////////////////////////////
/// \brief CreateConstraintBuilderOptions
...
proto::ConstraintBuilderOptions CreateConstraintBuilderOptions(
  ...
  options.set_sampling_ratio(parameter_dictionary->GetDouble("sampling_ratio"));        //采样比率
  options.set_max_constraint_distance(
      parameter_dictionary->GetDouble("max_constraint_distance"));                      //最大约束距离
  options.set_min_score(parameter_dictionary->GetDouble("min_score"));                  //最低分数
  options.set_global_localization_min_score(
      parameter_dictionary->GetDouble("global_localization_min_score"));                //全局定位最低分数
  options.set_loop_closure_translation_weight(
      parameter_dictionary->GetDouble("loop_closure_translation_weight"));              //回环 平移权重
  options.set_loop_closure_rotation_weight(
      parameter_dictionary->GetDouble("loop_closure_rotation_weight"));                 //回环 旋转权重
  ...
  *options.mutable_fast_correlative_scan_matcher_options() =
      scan_matching::CreateFastCorrelativeScanMatcherOptions2D(
          parameter_dictionary->GetDictionary("fast_correlative_scan_matcher")          //分支定界匹配器
              .get());
  *options.mutable_ceres_scan_matcher_options() =
      scan_matching::CreateCeresScanMatcherOptions2D(
          parameter_dictionary->GetDictionary("ceres_scan_matcher").get());             //ceres优化器
