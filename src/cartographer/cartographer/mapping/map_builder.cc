...
int MapBuilder::AddTrajectoryBuilder(
        const std::set<SensorId>& expected_sensor_ids,              //传感器ID {传感器id,传感器类型}
        const proto::TrajectoryBuilderOptions& trajectory_options,  //配置选项
        LocalSlamResultCallback local_slam_result_callback) {       //local SLAM的结果回调函数
    ...
    if (options_.use_trajectory_builder_3d()) {
        ...
        if (trajectory_options.has_trajectory_builder_3d_options()) {
            //构造LocalTrajectoryBuilder3D对象
            local_trajectory_builder = common::make_unique<LocalTrajectoryBuilder3D>(
                        trajectory_options.trajectory_builder_3d_options(), //配置选项
                        SelectRangeSensorIds(expected_sensor_ids)); // 从预期的传感器topic中选择激光雷达topic
        }
        DCHECK(dynamic_cast<PoseGraph3D*>(pose_graph_.get()));
        trajectory_builders_.push_back(
                    // 构造一个CollatedTrajectoryBuilder，用来整理传感器数据的
        ...
        ////////////////////////////////重点部分////////////////////////////////////////////
        // 2D 模式
        // 创建一个LocalTrajectoryBuilder2D对象, 用来进行local SLAM:
        // 1. 航迹推算
        // 2. 扫描匹配
        // 3. 激光插入子图
        std::unique_ptr<LocalTrajectoryBuilder2D> local_trajectory_builder;
        // 使用参数和传感器id\topic进行初始化 LocalTrajectoryBuilder2D
        ...
        // 先创建一个CollatedTrajectoryBuilder
        trajectory_builders_.push_back(
                    common::make_unique<CollatedTrajectoryBuilder>(
                        sensor_collator_.get(),                             //传感器数据收集器
                        trajectory_id, expected_sensor_ids,                 //轨迹id , 传感器id
                        CreateGlobalTrajectoryBuilder2D(                    //GlobalTrajectoryBuilder<LocalTrajectoryBuilder2D, mapping::PoseGraph2D>>
                            std::move(local_trajectory_builder),            //上面创建的LocalTrajectoryBuilder2D对象
                            trajectory_id,                                  //轨迹id
                            static_cast<PoseGraph2D*>(pose_graph_.get()),   //位姿图<PoseGraph2D*>
                            local_slam_result_callback)));                  //local slam结果回调函数


        // 针对二维建图还有一个配置项overlapping_submaps_trimmer_2d
        // 决定是否为pose_graph_对象添加一个OverlappingSubmapsTrimmer2D类型的修饰器
        // 用于根据子图之间重叠的部分修饰地图
        ///可暂不看
