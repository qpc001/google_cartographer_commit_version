    ...
    for (size_t i = 0; i < msg.ranges.size(); ++i) {
        ...
        if (HasEcho(echoes)) {
            ...
            if (msg.range_min <= first_echo && first_echo <= msg.range_max) {
                ...
                // first_echo * Eigen::Vector3f::UnitX() : 将激光束构成与机器人x轴平行的向量
                // 然后乘以旋转角度, 得到击中点(相对于激光传感器坐标系的)
...
ToPointCloudWithIntensities(const sensor_msgs::PointCloud2& message) {
    ...
    // 检查点云PointCloud2是否有强度的数据
    if (PointCloud2HasField(message, "intensity")) {
        // 有强度数据
        // 调用pcl,解析sensor_msgs::PointCloud2& message
        // 然后再将点云的各个点push进 自定义的点云类PointCloudWithIntensities
...
// 遍历landmark_list, 转换并返回LandmarkData结构的数据
LandmarkData ToLandmarkData(const LandmarkList& landmark_list) {
    LandmarkData landmark_data;
    // ros msg时间类型转换为 自定义时间类型
    landmark_data.time = FromRos(landmark_list.header.stamp);
    // 遍历landmark_list ,的每一个路标 entry
    // 向landmark_data添加(路标id, 从路标坐标系到tracking 坐标系的变换 , 平移量权重, 旋转量权重)
    // 添加观测, 观测为 从路标坐标系到tracking 坐标系的变换
...
cartographer::transform::Rigid3d ComputeLocalFrameFromLatLong(
        const double latitude, const double longitude) {    // latitude 纬度  longitude:经度

    // 经纬度转换为ECEF坐标
    const Eigen::Vector3d translation = LatLongAltToEcef(latitude, longitude, 0.);
    // ECEF坐标系先绕z轴旋转(-longitude)[负表示顺时针为正,这是逆时针]
    // 再沿旋转之后的Y轴旋转(latitude - 90.)
    // 得到的坐标系则 z轴的负方向指向地心, y轴正方向指向东边, x轴正方向指向南极(那一侧,并不是真的指向南极)
